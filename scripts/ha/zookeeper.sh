...
function setup_zookeeper()
{
    ...
    input_if_empty "CLUSTER_INDEX_ID" "ZooKeeper: Please Ender This Server Of Index In Cluster Like '1'"
    echo ${CLUSTER_INDEX_ID} > ${ZOOKEEPER_DATA_DIR}/myid
    ...
    exec_while_read "CLUSTER_CHILD_ADDRS" "ZooKeeper: Please Ender Cluster Child.\${I} Address Like '${LOCAL_HOST}'" "" "
        ...
        if [ \$CURRENT == \$CLUSTER_INDEX_ID ]; then
            CURRENT='0.0.0.0'
        fi
...
{
    set_environment
    #http://mirrors.hust.edu.cn/apache/zookeeper/stable/zookeeper-3.4.12.tar.gz
    setup_soft_wget "zookeeper" "https://archive.apache.org/dist/zookeeper/zookeeper-3.7.0/apache-zookeeper-3.7.0-bin.tar.gz" "setup_zookeeper"

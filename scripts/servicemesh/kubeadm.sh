...
function setup_kubeadm()
{
    ...
    #安装依赖
    find_url_list_newer_date_link_file "TMP_CRI_TOOLS_NEWER_FILE_NAME" "$TMP_MIRRORS_URL" "cri-tools.*.$CPU_ARCHITECTURE"
    ...
    find_url_list_newer_date_link_file "TMP_KUBELET_NEWER_FILE_NAME" "$TMP_MIRRORS_URL" "kubelet.*.$CPU_ARCHITECTURE"
    ...
    find_url_list_newer_date_link_file "TMP_KUBERNETES_CNI_NEWER_FILE_NAME" "$TMP_MIRRORS_URL" "kubernetes-cni.*.$CPU_ARCHITECTURE"
    ...
    find_url_list_newer_date_link_file "TMP_KUBEADM_NEWER_RPM_FILE_NAME" "$TMP_MIRRORS_URL" "kubeadm-${TMP_KUBEADM_NEWER_VERSION}\-[0-9]*\.$CPU_ARCHITECTURE"

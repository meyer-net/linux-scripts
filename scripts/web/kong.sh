...
local TMP_KNG_SETUP_API_HTTP_PORT=18444

local TMP_KNG_SETUP_WORKSPACE_ID="e4b9993d-653f-44fd-acc5-338ce807582c"
local TMP_KNG_SETUP_PSQL_HOST="127.0.0.1"
local TMP_KNG_SETUP_PSQL_PORT=15432
local TMP_KNG_SETUP_PSQL_ROOT_USRNAME="postgres"
local TMP_KNG_SETUP_PSQL_ROOT_USRPWD="123456"
...
function setup_kong()
{
    ...
    while_wget "${TMP_KNG_SETUP_RPM_NEWER}" "kong-${TMP_KNG_SETUP_NEWER}.amd64.rpm"
	...
	rm -rf kong-${TMP_KNG_SETUP_NEWER}.amd64.rpm
...
function conf_kong()
{
    ...
    # 开始配置
    # -- 初始化数据库，并设置密码
	input_if_empty "TMP_KNG_SETUP_PSQL_HOST" "PostgresQL: Please ender the ${red}root host address${reset} for kong"
	input_if_empty "TMP_KNG_SETUP_PSQL_PORT" "PostgresQL: Please ender the ${red}root port${reset} of '${TMP_KNG_SETUP_PSQL_HOST}' for kong"

	input_if_empty "TMP_KNG_SETUP_PSQL_ROOT_USRNAME" "PostgresQL: Please ender the ${red}root user name${reset} of '${TMP_KNG_SETUP_PSQL_HOST}:${TMP_KNG_SETUP_PSQL_PORT}' for kong"
	input_if_empty "TMP_KNG_SETUP_PSQL_ROOT_USRPWD" "PostgresQL: Please ender the ${red}root password${reset} of ${TMP_KNG_SETUP_PSQL_ROOT_USRNAME}@${TMP_KNG_SETUP_PSQL_HOST}:${TMP_KNG_SETUP_PSQL_PORT} for kong"
        
    local TMP_KNG_SETUP_PSQL_SELF_DATABASE="kong"
    local TMP_KNG_SETUP_PSQL_SELF_USRNAME="kong"
    local TMP_KNG_SETUP_PSQL_SELF_USRPWD="dbkng%1it"
	input_if_empty "TMP_KNG_SETUP_PSQL_SELF_DATABASE" "Kong.PostgresQL: Please ender ${red}kong database${reset} of '${TMP_KNG_SETUP_PSQL_HOST}:${TMP_KNG_SETUP_PSQL_PORT}'"
	input_if_empty "TMP_KNG_SETUP_PSQL_SELF_USRNAME" "Kong.PostgresQL: Please ender ${red}kong user name${reset} of '${TMP_KNG_SETUP_PSQL_HOST}:${TMP_KNG_SETUP_PSQL_PORT}'"
	input_if_empty "TMP_KNG_SETUP_PSQL_SELF_USRPWD" "Kong.PostgresQL: Please ender ${red}kong password${reset} of ${TMP_KNG_SETUP_PSQL_SELF_USRNAME}@${TMP_KNG_SETUP_PSQL_HOST}:${TMP_KNG_SETUP_PSQL_PORT}"
    ...
    sed -i "s@^#pg_port =.*@pg_port = ${TMP_KNG_SETUP_PSQL_DBPORT}@g" /etc/kong/kong.conf
	...
	input_if_empty "TMP_KNG_SETUP_AUTO_HTTPS_VLD_HOST" "Kong.AutoHttps: Please ender ${red}auto https valid api host for kong${reset} of 'caddy'"
...
psql -U ${TMP_KNG_SETUP_PSQL_ROOT_USRNAME} -h ${TMP_KNG_SETUP_PSQL_HOST} -p ${TMP_KNG_SETUP_PSQL_PORT} -d postgres << EOF
    ...
    INSERT INTO targets (id,created_at,upstream_id,target,weight,tags,ws_id) VALUES ('5055bc0a-bdd5-59cf-a5e6-c60f3b7d680c',to_timestamp('${LOCAL_TIME}', 'yyyy-MM-dd hh24:mi:ss') + INTERVAL '1 min','2d929958-f95d-5365-a997-055c26fd122d','${TMP_KNG_SETUP_AUTO_HTTPS_VLD_HOST}:2019',100,NULL, :'kong_workspace_id'); 
    ...
    INSERT INTO targets (id,created_at,upstream_id,target,weight,tags,ws_id) VALUES ('a5178e75-f6d7-59cf-bda8-290f892885ed',to_timestamp('${LOCAL_TIME}', 'yyyy-MM-dd hh24:mi:ss') + INTERVAL '2 min','9d68b631-2c02-5506-9e88-53e6d7975ea6','${TMP_KNG_SETUP_AUTO_HTTPS_VLD_HOST}:${TMP_KNG_SETUP_AUTO_HTTPS_VLD_PORT}',100,NULL, :'kong_workspace_id');
    ...
    INSERT INTO targets (id,created_at,upstream_id,target,weight,tags,ws_id) VALUES ('07232e9f-f860-5659-afcd-cafb8580e6f6',to_timestamp('${LOCAL_TIME}', 'yyyy-MM-dd hh24:mi:ss') + INTERVAL '3 min','0a79e2bc-6fc3-5d59-bbfa-cc733f836935','${TMP_KNG_SETUP_AUTO_HTTPS_VLD_HOST}:9000',100,NULL, :'kong_workspace_id'); 
...
function boot_kong()
{
	...
	echo_soft_port 80
	echo_soft_port 443
	echo_soft_port ${TMP_KNG_SETUP_API_HTTP_PORT} "127.0.0.1"
	echo_soft_port ${TMP_KNG_SETUP_API_HTTPS_PORT} "127.0.0.1"
